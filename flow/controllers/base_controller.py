...
class BaseController:
    ...
    car_following_params : flow.core.params.SumoCarFollowingParams
        The underlying SUMO model for cars will be overwritten. A Flow
        controller will override the behavior of this SUMO car-following
        model; however, if control is ceded back to SUMO, the car will
        use these parameters. Ensure that accel / decel parameters that are
    ...
    def get_action(self, env):
        ...
        If None is returned, in case of no acceleration is specified or other conditions,
        SUMO will control the accelerations for the current time step.

        This method also augments a customized controller with the desired level of
        ...
        # a network and its data is yet subscribed.
    ...
    def get_safe_action_instantaneous(self, env, action):
        ...
        Instantaneously stops the car if there is a chance of colliding into
        the leading vehicle in the next step

        The parameter "action" meaning the acceleration generated by a customized controller.
        ...
        # Compute the velocity of the ego vehicle in the next time step
        next_vel = this_vel + action * sim_step

        # Get the space headway
        h = env.k.vehicle.get_headway(self.veh_id)

        # If the velocity of the ego vehicle at next time step is greater than 0
        if next_vel > 0:
    ...
    def get_safe_velocity_action(self, env, action):
        ...
        Checks if the computed acceleration would exceed the safe velocity.
        If so, return the acceleration that would put us back to the safe velocity.
        ...
        if env.k.vehicle.num_vehicles == 1:
            ...
            if this_vel + action * sim_step > safe_velocity:
                if safe_velocity > 0:
                    return (safe_velocity - this_vel) / sim_step # decelerate to the safe velocity for the next step
                else:
                    return -this_vel / sim_step # stop the car immediately
            else:
                return action
    ...
    def safe_velocity(self, env):
        ...
        Finds maximum velocity such that if the leading vehicle were to stop
