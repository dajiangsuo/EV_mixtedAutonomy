...
class KernelSimulation(object):
    ...
    passing to all other kernels the API so that they can interact with the
    simulation.

    The simulation kernel is also responsible for advancing, resetting, and
    storing relevant simulation data.
    ...
    def start_simulation(self, network, sim_params):
        ...
        network : any
            an object or variable that is meant to symbolize the network being
            used during the simulation. For example, in the case of SUMO
    ...
    def update(self, reset):
        ...
        Any update operations are meant to facilitate simulation in
